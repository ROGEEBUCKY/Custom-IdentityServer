...
namespace CustomIdentityServer.Pages.ExternalLogin
{
    [AllowAnonymous]
    [SecurityHeaders]
    public class Callback : PageModel
    {
        ...
        public async Task<IActionResult> OnGet()
        {
            ...
            await _events.RaiseAsync(new UserLoginSuccessEvent(provider, providerUserId, user.UserID.ToString(), user.Name, true, context?.Client.ClientId));
        ...
        private async Task<User> AutoProvisionUserAsync(string provider, string providerUserId, IEnumerable<Claim> claims)
        {
            ...
            {
                UserID =0,
                Name = sub.ToString(), // don't need a username, since the user will be using an external provider to login
            };
